...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        x += idy if self.i_downsample else 0
        x=self.relu(x)
...
class ResNet(nn.Module):
    def __init__(self, block, layer_list, num_classes, num_channels=3):
        ...
        self.layer1 = self.make_layer(block, layer_list[0], planes=64)
        self.layer2 = self.make_layer(block, layer_list[1], planes=128, stride=2)
        self.layer3 = self.make_layer(block, layer_list[2], planes=256, stride=2)
        self.layer4 = self.make_layer(block, layer_list[3], planes=512, stride=2)
    ...
    def make_layer(self, block, blocks, planes, stride=1):
        ii_downsample = None
        layers = []
