...
# VIT-B16 uses 16X16 patches, VIT-B32 uses 32X32 patchs.
class ViT(nn.Module):
...
class ViTClassifier(nn.Module):
    def __init__(self, sample_img, num_classes, d_model=768, pretrained=None):
        super().__init__()
        self.vit = build_transformer(sample_img)  # Make sure build_transformer is defined elsewhere
        self.mlp_head = nn.Linear(d_model, num_classes)  # d_model should match your transformer config
...
class InputEmbedding(nn.Module):
    def __init__(self, img_size: int, in_channels: int, patch_size: int, d_model: int):
        ...
        self.d_model = d_model # Understand D_MODEL!
    ...
    def forward(self, x):
        ...
        # Flatten each image from 2Dx3 channels to 1D
        x = self.proj(x)

        # Apply class token to beginning of each patch group
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # For each patch, add a random value as it's positional encoding.
        self.positional_encoding = nn.Parameter(torch.randn(1, seq_len + 1, d_model))

    def forward(self, x):
        # Wrap positional encoding with dropout (improves learning speed and accuracy), x + pos_en creates unique value (?)
...
class FeedForwardNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # also torch.nn.functional.gelu() can be used directly, instead of assigning variable for relu.
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(q, k, v, mask, d_k, d_model, dropout):
        ...
        return attention_result, attention
    ...
    def forward(self, q, k, v, mask):
        ...
        attn_result, attn_weights = MultiHeadAttentionBlock.attention( 
            query, key, value, mask, self.d_k, self.d_model, self.dropout
        )
        
        self.attn_weights = attn_weights.detach()  # (batch, heads, seq_len, seq_len)

        return self.W_o(attn_result)
    

class ResidualConnection(nn.Module):
    def __init__(self, dropout: float, d_model=768):
...
# Allows for more than one encoder block
class Encoder(nn.Module):
    def __init__(self, layers: nn.ModuleList, d_model=768):
